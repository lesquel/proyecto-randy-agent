...
export default function RegisterPage() {
  ...
  return (
    <main className="relative mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center px-5 py-10">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_30%_30%,hsl(var(--primary)/0.18),transparent_60%),radial-gradient(circle_at_70%_70%,hsl(var(--accent)/0.28),transparent_55%)]" />
      <div className="bg-background/60 w-full overflow-hidden rounded-xl border p-6 shadow-lg backdrop-blur-md">
        <div className="mb-6 space-y-1 text-center">
          ...
          <p className="text-muted-foreground text-xs">
            Crea tu identidad local (no se envía a ningún servidor)
          </p>
        </div>
        <form onSubmit={submit} className="space-y-5">
          ...
          {ok && (
            <p className="text-xs text-green-500">Guardado, redirigiendo...</p>
          )}
          <button
            type="submit"
          ...
          >
            <span className="relative z-10">Crear sesión local</span>
